...
contract Bossowned {
    ...
    string[99] public pixelcoordinat;                  // координаты пикселей
    string[99] public ownerpixel;                      // владелец конкретног
    ...
    function vivodRegion(uint k) public constant returns (string) {
        return regionId[k];
    }
...
contract Firstsale is Bossowned {
    ...
    function addRegion(uint x1, uint y1, uint x2, uint y2) public onlyOwner {
        ...
        */
        ...
        itog1 = "x1=" + x1 + "x2=" + x2;
        itog2 = itog1 + "y1=" + y1 + "y2=" + y2;
        */
        if (st == ploshad) {
            regionId[regionId.length] = "x1";
        }
    }
}
